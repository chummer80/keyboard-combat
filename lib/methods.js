Meteor.methods({
	...
	setCharAnim: function(gameId, playerIndex, anim) {
		var animUpdateObject = {};
		animUpdateObject["players." + playerIndex + ".anim"] = anim;
		Games.update(
			{_id: gameId},
			{$set: animUpdateObject},
			{multi: false}
		);
	},
	...
	setGameStatus: function(gameId, status) {
		Games.update({_id: gameId}, {$set: {status: status}});
	}
});